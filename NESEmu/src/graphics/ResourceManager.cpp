...
bool ResourceManager::loadShader(string name, string fragmentPath, string vertexPath)
{
	if (getShader(name))
	{
	...
	printf("Loaded shader '%s' from '%s', '%s'\n", name.c_str(), fragmentPath.c_str(), vertexPath.c_str());
	return true;
}

bool ResourceManager::loadTexture(string name, string shaderName, int width, int height)
{
	if (getTexture(name))
	{
	...
	{
		printf("[Error] Failed to load texture '%s' - shader '%s' doesn't exist\n", shaderName.c_str(), name.c_str());
		return false;
	}
