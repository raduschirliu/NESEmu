...
CartridgeDebugWindow::CartridgeDebugWindow(Cartridge &cartridge) : Window(GLFW_KEY_F4), cartridge(cartridge)
{
}
...
void CartridgeDebugWindow::draw()
{
	...
	ImGui::Text("PRG ROM: %u banks (%u bytes)", header.prgBanks, header.prgBanks * Cartridge::PRG_BANK_SIZE);
	ImGui::Text("CHR ROM: %u banks (%u bytes)", header.chrBanks, header.chrBanks * Cartridge::CHR_BANK_SIZE);
	ImGui::Spacing();
	...
	if (ImGui::CollapsingHeader("ROM Header"))
	{
		...
		ImGui::Text("Mapper Lower Nibble: $%X", header.flags6.mapperLowerNibble);
		ImGui::Unindent();
		...
		ImGui::Text("Mapper Upper Nibble: $%X", header.flags7.mapperUpperNibble);
		ImGui::Unindent();

		ImGui::Text("PRG RAM Size: %u bytes", header.prgRamSize);
		ImGui::Text("TV Flags 1: %s", utils::toBitString(header.tvFlags1).c_str());
		ImGui::Text("TV Flags 2: %s", utils::toBitString(header.tvFlags2).c_str());
