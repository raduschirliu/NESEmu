...
void PPUDebugWindow::Draw()
{
    ...
    if (ImGui::BeginTabBar("PPU Views", ImGuiTabBarFlags_None))
    {
        ...
        if (ImGui::BeginTabItem("Registers"))
        {
            ...
            drawRegister("PPUSTATUS", 0x2002, &registers->status,
                         STATUS_HELP_TEXT);
        ...
        {
            PPU::Registers *registers = ppu.getRegisters();
            ImGui::Text("Background table:\t%u ($%X)",
                        registers->ctrl.bgPatternTable,
                        ppu.getActiveBgPatternTableAddress());
            ImGui::Text("Sprite table:\t\t%u ($%X)",
                        registers->ctrl.spritePatternTable,
                        ppu.getActiveSpritePatternTableAddress());
        ...
        if (ImGui::BeginTabItem("Palette Table"))
        {
            ...
            drawPalette("Background 0",
                        ppu.getPalette(0x3F01));  // Background 0
            ImGui::SameLine();
            drawPalette("Background 1",
                        ppu.getPalette(0x3F05));  // Background 1
            ImGui::SameLine();
            drawPalette("Background 2",
                        ppu.getPalette(0x3F09));  // Background 2
            ImGui::SameLine();
            drawPalette("Background 3",
                        ppu.getPalette(0x3F0D));  // Background 3
        ...
        if (ImGui::BeginTabItem("Nametables"))
        {
            ...
            ImGui::SameLine();
            ImGui::RadioButton("1", &debugViewNametable, 0);
            ImGui::SameLine();
            ImGui::RadioButton("2", &debugViewNametable, 1);
            ImGui::SameLine();
            ImGui::RadioButton("3", &debugViewNametable, 2);
            ImGui::SameLine();
...
void PPUDebugWindow::drawRegister(string name, uint16_t address,
                                  const void *reg, const char *helpText)
{
...
void PPUDebugWindow::drawPalette(string label, std::vector<PPU::Color> palette)
{
    ...
    for (PPU::Color color : palette)
    {
        ...
        drawList->AddRectFilled(ImVec2(pos.x, pos.y),
                                ImVec2(pos.x + size, pos.y + size), imColor);

        ss << "0x" << std::hex << std::setw(2) << std::setfill('0') << index;
...
void PPUDebugWindow::drawNametable(uint8_t nametable)
{
    ...
    PPU::Registers *registers = ppu.getRegisters();
    Texture *bgTable = registers->ctrl.bgPatternTable == 0 ? patternTableLeft
                                                           : patternTableRight;
    ...
    for (uint16_t r = 0; r < PPU::NAMETABLE_ROWS; r++)
    {
        for (uint16_t c = 0; c < PPU::NAMETABLE_COLS; c++)
        {
            ...
            uint8_t paletteIndex =
                ppu.getNametableEntryPalette(nametable, offset);
...
{
    PPU::Registers *registers = ppu.getRegisters();
    ImGui::Text("Sprite table: %u ($%X)", registers->ctrl.spritePatternTable,
                ppu.getActiveSpritePatternTableAddress());
    ImGui::Spacing();
    ...
    for (int i = 0; i < PPU::OAM_ENTRIES; i++)
    {
        ...
        uint8_t *attributeByte =
            reinterpret_cast<uint8_t *>(&sprite->attributes);
        Texture *spriteTable = registers->ctrl.spritePatternTable == 0
                                   ? patternTableLeft
                                   : patternTableRight;

        ImVec2 uvStart(std::floor(sprite->tileIndex % PPU::PATTERN_TABLE_SIZE *
                                  PPU::TILE_SIZE),
                       std::floor(sprite->tileIndex / PPU::PATTERN_TABLE_SIZE *
                                  PPU::TILE_SIZE));
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            glm::vec2 screenPos =
                glm::vec2(offset.x + sprite->xPos * renderingScale,
                          offset.y + sprite->yPos * renderingScale);

            ImGui::BeginTooltip();
            ImGui::Text("OAM Index:  %u", i);
            ImGui::Text("Pattern:    $%X", sprite->tileIndex);
            ImGui::Text("Pos:        %d, %d ($%X, $%X)", sprite->xPos,
                        sprite->yPos, sprite->xPos, sprite->yPos);
