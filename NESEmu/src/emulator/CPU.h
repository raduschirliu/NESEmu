...
{
  public:
    // An emun represantation of the 8 status flags used for the 6502's status
    // (p) register
    ...
    // A representation of the an instruction and it's associated addressing
    // mode
    struct Instruction
    {
  ...
  private:
    ...
    int IMP(), ACC(), IMM(), ZPG(), ZPX(), ZPY(), REL(), ABS(), ABX(), ABY(),
        IND(), IDX(), IDY();
    ...
    int ADC(), AND(), ASL(), BCC(), BCS(), BEQ(), BIT(), BMI(), BNE(), BPL(),
