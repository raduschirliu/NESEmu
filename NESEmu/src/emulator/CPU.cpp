...
#define _I(NAME, RUN, MODE, CYCLES)         \
    {                                       \
        NAME, &CPU::RUN, &CPU::MODE, CYCLES \
    }
#define _XXX(MODE, CYCLES)                   \
    {                                        \
        "XXX", &CPU::XXX, &CPU::MODE, CYCLES \
    }
#define _NOP(MODE, CYCLES)                    \
    {                                         \
        "NOP*", &CPU::NOP, &CPU::MODE, CYCLES \
    }

// Since the SP is only 8-bit, and the stack starts at 0x0100, this is a utility
// to calculate the full SP address
#define SP_ADDRESS (sp + 0x0100)

// Initialize CPU
CPU::CPU(Bus &bus) : logger("..\\logs\\cpu.log"), bus(bus)
{
    a = 0x00;
    x = 0x00;
    y = 0x00;
    ...
    instructions = {
        ...
        //        -5                      -6                 -7 -8 -9 -A -B -C
        //        -D                      -E                 -F
        _I("BRK", BRK, IMP, 7), _I("ORA", ORA, IDX, 6),
        _XXX(IMM, 2),           _XXX(ZPX, 3),
        _NOP(ZPG, 3),           _I("ORA", ORA, ZPG, 3),
        _I("ASL", ASL, ZPG, 5), _XXX(ZPG, 5),
        _I("PHP", PHP, IMP, 3), _I("ORA", ORA, IMM, 2),
        _I("ASL", ASL, ACC, 2), _XXX(IMM, 2),
        _NOP(ABS, 4),           _I("ORA", ORA, ABS, 4),
        _I("ASL", ASL, ABS, 6), _XXX(ABS, 6),
        _I("BPL", BPL, REL, 2), _I("ORA", ORA, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _NOP(ZPX, 4),           _I("ORA", ORA, ZPX, 4),
        _I("ASL", ASL, ZPX, 6), _XXX(ZPX, 6),
        _I("CLC", CLC, IMP, 2), _I("ORA", ORA, ABY, 4),
        _NOP(IMP, 2),           _XXX(ABY, 7),
        _NOP(ABX, 4),           _I("ORA", ORA, ABX, 4),
        _I("ASL", ASL, ABX, 7), _XXX(ABX, 7),
        _I("JSR", JSR, ABS, 6), _I("AND", AND, IDX, 6),
        _XXX(IMM, 2),           _XXX(ZPX, 3),
        _I("BIT", BIT, ZPG, 3), _I("AND", AND, ZPG, 3),
        _I("ROL", ROL, ZPG, 5), _XXX(ZPG, 5),
        _I("PLP", PLP, IMP, 4), _I("AND", AND, IMM, 2),
        _I("ROL", ROL, ACC, 2), _XXX(IMM, 2),
        _I("BIT", BIT, ABS, 4), _I("AND", AND, ABS, 4),
        _I("ROL", ROL, ABS, 6), _XXX(ABS, 6),
        _I("BMI", BMI, REL, 2), _I("AND", AND, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _NOP(ZPX, 4),           _I("AND", AND, ZPX, 4),
        _I("ROL", ROL, ZPX, 6), _XXX(ZPX, 6),
        _I("SEC", SEC, IMP, 2), _I("AND", AND, ABY, 4),
        _NOP(IMP, 2),           _XXX(ABY, 7),
        _NOP(ABX, 4),           _I("AND", AND, ABX, 4),
        _I("ROL", ROL, ABX, 7), _XXX(ABX, 7),
        _I("RTI", RTI, IMP, 6), _I("EOR", EOR, IDX, 6),
        _XXX(IMM, 2),           _XXX(ZPX, 3),
        _NOP(ZPG, 3),           _I("EOR", EOR, ZPG, 3),
        _I("LSR", LSR, ZPG, 5), _XXX(ZPG, 5),
        _I("PHA", PHA, IMP, 3), _I("EOR", EOR, IMM, 2),
        _I("LSR", LSR, ACC, 2), _XXX(IMM, 2),
        _I("JMP", JMP, ABS, 3), _I("EOR", EOR, ABS, 4),
        _I("LSR", LSR, ABS, 6), _XXX(ABS, 6),
        _I("BVC", BVC, REL, 2), _I("EOR", EOR, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _NOP(ZPX, 4),           _I("EOR", EOR, ZPX, 4),
        _I("LSR", LSR, ZPX, 6), _XXX(ZPX, 6),
        _I("CLI", CLI, IMP, 2), _I("EOR", EOR, ABY, 4),
        _NOP(IMP, 2),           _XXX(ABY, 7),
        _NOP(ABX, 4),           _I("EOR", EOR, ABX, 4),
        _I("LSR", LSR, ABX, 7), _XXX(ABX, 7),
        _I("RTS", RTS, IMP, 6), _I("ADC", ADC, IDX, 6),
        _XXX(IMM, 2),           _XXX(ZPX, 3),
        _NOP(ZPG, 3),           _I("ADC", ADC, ZPG, 3),
        _I("ROR", ROR, ZPG, 5), _XXX(ZPG, 5),
        _I("PLA", PLA, IMP, 4), _I("ADC", ADC, IMM, 2),
        _I("ROR", ROR, ACC, 2), _XXX(IMM, 2),
        _I("JMP", JMP, IND, 5), _I("ADC", ADC, ABS, 4),
        _I("ROR", ROR, ABS, 4), _XXX(ABS, 6),
        _I("BVS", BVS, REL, 2), _I("ADC", ADC, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _NOP(ZPX, 3),           _I("ADC", ADC, ZPX, 4),
        _I("ROR", ROR, ZPX, 6), _XXX(ZPX, 6),
        _I("SEI", SEI, IMP, 2), _I("ADC", ADC, ABY, 4),
        _NOP(IMP, 2),           _XXX(ABY, 7),
        _NOP(ABX, 4),           _I("ADC", ADC, ABX, 4),
        _I("ROR", ROR, ABX, 7), _XXX(ABX, 7),
        _NOP(IMM, 2),           _I("STA", STA, IDX, 6),
        _NOP(IMM, 2),           _XXX(ZPX, 3),
        _I("STY", STY, ZPG, 3), _I("STA", STA, ZPG, 3),
        _I("STX", STX, ZPG, 3), _XXX(ZPG, 5),
        _I("DEY", DEY, IMP, 2), _NOP(IMM, 2),
        _I("TXA", TXA, IMP, 2), _XXX(IMM, 2),
        _I("STY", STY, ABS, 4), _I("STA", STA, ABS, 4),
        _I("STX", STX, ABS, 4), _XXX(ABS, 4),
        _I("BCC", BCC, REL, 2), _I("STA", STA, IDY, 6),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _I("STY", STY, ZPX, 4), _I("STA", STA, ZPX, 4),
        _I("STX", STX, ZPY, 4), _XXX(ZPX, 6),
        _I("TYA", TYA, IMP, 2), _I("STA", STA, ABY, 5),
        _I("TXS", TXS, IMP, 2), _XXX(ABY, 5),
        _XXX(ABX, 5),           _I("STA", STA, ABX, 5),
        _XXX(ABY, 5),           _XXX(ABY, 5),
        _I("LDY", LDY, IMM, 2), _I("LDA", LDA, IDX, 6),
        _I("LDX", LDX, IMM, 2), _XXX(ZPX, 3),
        _I("LDY", LDY, ZPG, 3), _I("LDA", LDA, ZPG, 3),
        _I("LDX", LDX, ZPG, 3), _XXX(ZPG, 5),
        _I("TAY", TAY, IMP, 2), _I("LDA", LDA, IMM, 2),
        _I("TAX", TAX, IMP, 2), _XXX(IMM, 2),
        _I("LDY", LDY, ABS, 4), _I("LDA", LDA, ABS, 4),
        _I("LDX", LDX, ABS, 4), _XXX(ABS, 4),
        _I("BCS", BCS, REL, 2), _I("LDA", LDA, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _I("LDY", LDY, ZPX, 4), _I("LDA", LDA, ZPX, 4),
        _I("LDX", LDX, ZPY, 4), _XXX(ZPX, 6),
        _I("CLV", CLV, IMP, 2), _I("LDA", LDA, ABY, 4),
        _I("TSX", TSX, IMP, 2), _XXX(ABY, 7),
        _I("LDY", LDY, ABX, 4), _I("LDA", LDA, ABX, 4),
        _I("LDX", LDX, ABY, 4), _XXX(ABY, 4),
        _I("CPY", CPY, IMM, 2), _I("CMP", CMP, IDX, 6),
        _NOP(IMM, 2),           _XXX(ZPX, 3),
        _I("CPY", CPY, ZPG, 3), _I("CMP", CMP, ZPG, 3),
        _I("DEC", DEC, ZPG, 5), _XXX(ZPG, 5),
        _I("INY", INY, IMP, 2), _I("CMP", CMP, IMM, 2),
        _I("DEX", DEX, IMP, 2), _XXX(IMM, 2),
        _I("CPY", CPY, ABS, 4), _I("CMP", CMP, ABS, 4),
        _I("DEC", DEC, ABS, 6), _XXX(ABS, 6),
        _I("BNE", BNE, REL, 2), _I("CMP", CMP, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _NOP(ZPX, 4),           _I("CMP", CMP, ZPX, 4),
        _I("DEC", DEC, ZPX, 6), _XXX(ZPX, 6),
        _I("CLD", CLD, IMP, 2), _I("CMP", CMP, ABY, 4),
        _NOP(IMP, 2),           _XXX(ABY, 7),
        _NOP(ABX, 4),           _I("CMP", CMP, ABX, 4),
        _I("DEC", DEC, ABX, 7), _XXX(ABX, 7),
        _I("CPX", CPX, IMM, 2), _I("SBC", SBC, IDX, 6),
        _NOP(IMM, 2),           _XXX(ZPX, 3),
        _I("CPX", CPX, ZPG, 3), _I("SBC", SBC, ZPG, 3),
        _I("INC", INC, ZPG, 5), _XXX(ZPG, 5),
        _I("INX", INX, IMP, 2), _I("SBC", SBC, IMM, 2),
        _I("NOP", NOP, IMP, 2), _XXX(IMM, 2),
        _I("CPX", CPX, ABS, 4), _I("SBC", SBC, ABS, 4),
        _I("INC", INC, ABS, 6), _XXX(ABS, 6),
        _I("BEQ", BEQ, REL, 2), _I("SBC", SBC, IDY, 5),
        _XXX(IMM, 2),           _XXX(ZPY, 3),
        _NOP(ZPX, 4),           _I("SBC", SBC, ZPX, 4),
        _I("INC", INC, ZPX, 6), _XXX(ZPX, 6),
        _I("SED", SED, IMP, 2), _I("SBC", SBC, ABY, 4),
        _NOP(IMP, 2),           _XXX(ABY, 7),
        _NOP(ABS, 4),           _I("SBC", SBC, ABX, 4),
        _I("INC", INC, ABX, 7), _XXX(ABX, 7)};
}
...
// op6 -> V, op7 -> N, A and M; (NZV); Operand bits 6/7 transfered to status
// bits 6/7. Operand anded with accumulator
int CPU::BIT()
{
    uint8_t operandVal = readOperand();
...
int CPU::SBC()
{
    // To perform subtraction, can invert all bits of operand (giving -operand -
    // 1) and pass to ADC
